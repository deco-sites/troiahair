...
function ProductGallery({ products, pageInfo, layout, offset, url, whatsappNumber, imageNotFound, }: Props) {
    ...
    return (<div class="w-full">
      ...
      <div class={`grid ${mobile} justify-center items-center ${desktop} lg:gap-10 gap-3`}>
        {products?.map((product, index) => (<ProductCard key={`product-card-${product.productID}`} product={product} preload={index === 0} index={offset + index} platform={platform} whatsappNumber={whatsappNumber} imageNotFound={imageNotFound}/>))}
      </div>
      {
